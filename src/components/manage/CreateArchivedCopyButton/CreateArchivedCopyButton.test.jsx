...
jest.mock('@plone/volto/helpers', () => ({
  ...
  expandToBackendURL: (url) => url.replace('http://localhost:3000', 'http://localhost:8080/Plone'),
}));
...
describe('CreateArchivedCopyButton', () => {
  it('returns null when user is not logged in', () => {
    ...
    const { container } = renderWithProviders(<CreateArchivedCopyButton />, store);
  ...
  it('renders the button for a published indicator with a logged-in user', () => {
    ...
    const { container } = renderWithProviders(<CreateArchivedCopyButton />, store);
    expect(container.querySelector('#create-archived-copy-btn')).toBeInTheDocument();
    expect(container.querySelector('#create-archived-copy-btn').title).toBe('Create an archived copy');
  });

  it('opens modal when button is clicked', () => {
    const store = makeStore();
    const { container, getByText } = renderWithProviders(<CreateArchivedCopyButton />, store);
  ...
  it('closes modal on cancel', () => {
    const store = makeStore();
    const { container, getByText, queryByText } = renderWithProviders(<CreateArchivedCopyButton />, store);
    fireEvent.click(container.querySelector('#create-archived-copy-btn'));
    expect(getByText('Create an archived copy')).toBeInTheDocument();
